...
export const EXA = bech32Chain("EXA", "exa");

export const ethToExa = (ethAddress: string) => {
  const data = ETH.decoder(ethAddress.toLowerCase());
  return EXA.encoder(data).toLowerCase();
};

export const exaToEth = (exaAddress: string) => {
  const data = EXA.decoder(exaAddress.toLowerCase());
